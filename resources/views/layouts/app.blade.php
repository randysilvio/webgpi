...
<head>
    ...
    <style>
        ...
        /* GLOBAL FIX: Memunculkan garis kotak input di semua form */
...
<body class="h-full flex flex-col antialiased text-slate-800 bg-slate-50 font-sans">
    ...
    <div class="flex-1 flex h-full overflow-hidden relative">
        ...
        <aside class="fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 text-slate-400 flex flex-col transition-transform duration-300 ease-in-out md:translate-x-0 -translate-x-full shadow-2xl" id="sidebar">
            ...
            {{-- NAVIGATION (Diberi ID 'sidebar-nav' untuk tracking posisi scroll) --}}
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            // LOGIKA PENAHAN SCROLL (Menyimpan Posisi Scroll dengan Jeda)
            const sidebarNav = document.getElementById('sidebar-nav');
            if (sidebarNav) {
                // 1. Ambil posisi terakhir dari memory saat halaman dimuat
                const scrollPos = localStorage.getItem('sidebar-scroll-pos');
                if (scrollPos) {
                    // Beri jeda 100 milidetik agar animasi menu terbuka selesai dulu
                    setTimeout(() => {
                        sidebarNav.scrollTop = parseInt(scrollPos, 10);
                    }, 100);
                }

                // 2. Simpan posisi scroll tepat sebelum pindah ke halaman lain
