...
• Poin kedua
• Poin ketiga"
                >{{ old('list_items', $service->list_items) }}</textarea>
                ...
                <div>
                    ...
                    <select id="color_theme" name="color_theme" required class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-primary focus:border-primary shadow-sm text-sm bg-white @error('color_theme') border-red-500 @enderror">
                        @foreach ($themes as $theme)
                            <option value="{{ $theme }}" {{ old('color_theme', $service->color_theme) == $theme ? 'selected' : '' }} class="capitalize">{{ ucfirst($theme) }}</option>
                        @endforeach
                    </select>
                 ...
                 <div>
                    ...
                    <input type="number" id="order" name="order" value="{{ old('order', $service->order) }}" min="0" required
