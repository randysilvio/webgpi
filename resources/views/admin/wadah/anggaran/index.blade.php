...
@section('title', 'Anggaran & Transaksi')
@section('header-title', 'Anggaran & Realisasi Wadah (RAB)')
...
@section('content')
    <div class="mb-6 flex justify-between items-center">
        ...
        <a href="{{ route('admin.wadah.anggaran.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded shadow-sm text-sm">
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6 p-6">
        <form method="GET" action="{{ route('admin.wadah.anggaran.index') }}" class="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div>
                ...
                <select name="wadah" class="block w-full border-gray-300 rounded-md shadow-sm text-sm">
                    <option value="">- Semua -</option>
            ...
            <div>
                ...
                <select name="jenis" class="block w-full border-gray-300 rounded-md shadow-sm text-sm">
                    <option value="">- Semua -</option>
            ...
            <div class="flex items-end">
                <button type="submit" class="w-full bg-gray-800 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded text-sm h-[38px]">Filter</button>
            </div>
        </form>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Pos Anggaran</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Jenis</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Target (Rp)</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Realisasi (Rp)</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Capaian</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr>
                            <td class="px-6 py-4 text-sm text-gray-900">
                                ...
                                <div class="text-xs text-gray-500">
                                    {{ $a->tahun_anggaran }} | {{ $a->jenisWadah->nama_wadah }}
                                    @if($a->programKerja)
                                        <span class="text-blue-600 block mt-1"><i class="fas fa-link mr-1"></i> {{ Str::limit($a->programKerja->nama_program, 20) }}</span>
                                    @endif
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-sm">{{ number_format($a->jumlah_target, 0, ',', '.') }}</td>
                            <td class="px-6 py-4 text-sm font-bold">{{ number_format($a->jumlah_realisasi, 0, ',', '.') }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                                <a href="{{ route('admin.wadah.anggaran.show', $a->id) }}" class="text-blue-600 hover:text-blue-900 mr-3" title="Transaksi"><i class="fas fa-list"></i></a>
                    ...
                    @empty
                        <tr><td colspan="6" class="px-6 py-6 text-center text-gray-500">Belum ada pos anggaran.</td></tr>
                    @endforelse
                </tbody>
            </table>
            <div class="p-4">{{ $anggarans->links() }}</div>
        </div>
    </div>
@endsection